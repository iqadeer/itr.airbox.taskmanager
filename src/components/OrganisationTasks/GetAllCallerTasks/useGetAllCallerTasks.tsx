    ...
    if (isStateValid()) {
      try {
        ...
        const result = await APIService.get<AbxTaskGetAllResponse>(
          `tasks/organisations/${state.organisationId}/${state.callerId}/`,
          'token',
          config
        );
        setState({
        ...
        setSnackBar({ message: `Error getting caller's tasks: ${e}`, severity: 'error' });
        setOpenSnackBar(true);
      }
    }
  };
...
export default useGetAllCallerTasks;
